...
SYNOPSIS
    ...
    pypy pyVarcheck2.py -i <input.mpileup.gz> -o <output.gz> -h <help>
                            or
    pypy pyVarcheck2.py -o <output.gz> <input.mpileup.gz>

DESCRIPTION

   Tab delimited output file with:

   chromosome, position, ref_base, depth, A|T|C|G|.|,  mutations, mutation fractions, support reads, chain fractions, support reads for each chain

EXAMPLES

    pypy pyVarcheck2.py -h
    pypy pyVarcheck2.py -i input.mpileup.gz -o output.varcheck.gz
    pypy pyVarcheck2.py -o output.varcheck.gz input.mpileup.gz
...
def parse_SER_info (info_string):
    temp = info_string
    dic = {}
    dic.setdefault('A',str(len(re.findall(r'\^\S[Aa]', temp))+len(re.findall(r'[Aa]\$', temp))))
    dic.setdefault('T',str(len(re.findall(r'\^\S[Tt]', temp))+len(re.findall(r'[Tt]\$', temp))))
    dic.setdefault('G',str(len(re.findall(r'\^\S[Gg]', temp))+len(re.findall(r'[Gg]\$', temp))))
    dic.setdefault('C',str(len(re.findall(r'\^\S[Cc]', temp))+len(re.findall(r'[Cc]\$', temp))))
    dic.setdefault('p',str(len(re.findall(r'\^\S\.', temp))+len(re.findall(r'\.\$', temp))))
    dic.setdefault('n',str(len(re.findall(r'\^\S\,', temp))+len(re.findall(r',\$', temp))))
    SER_info_string = dic['A']+'|'+dic['T']+'|'+dic['C']+'|'+dic['G']+'|'+dic['p']+'|'+dic['n'] 
    return SER_info_string
...
def parse_info(info_string, ref, depth):
    ...
    x = 0
    temp = info_string
    SER_info_string = parse_SER_info(temp)

    #reads_start_count = temp.count('^')
    #reads_end_count = temp.count('$')
    #start_end_info = str(reads_start_count)+'|'+str(reads_end_count)+'|'+str(reads_start_count+reads_end_count)
    ...
    info = SER_info_string+'\t'+name + '\t' + freq + '\t' + support + '\t' + chainF + '\t' + chainS + '\n'
    return info
...
def main():
    ...
    for op, value in opts:
        if op == "-h":
            print('Usage: pypy pyVarcheck2.py -i <input.mpileup.gz> -o <output.gz> -h <help>')
    ...
    try:
        ...
        print('Error, try to use ""pypy pyVarcheck22.py -i /path/file.mpileup.gz ""')
        sys.exit(1)
